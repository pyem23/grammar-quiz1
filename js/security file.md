...
class GrammarQuizApp {
  ...
  setupEventListeners() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (e.target.id === 'lockAnswerBtn') this.showLockConfirmation();
      ...
      if (e.target.id === 'resetStudentOptBtn') this.showStudentList();
      if (e.target.id === 'resetClassOptBtn') this.showClassList();
      if (e.target.id === 'resetAllOptBtn') this.showResetAllConfirmation();
      if (e.target.id === 'resetBackBtn') this.showResetOptions();
      if (e.target.id === 'studentListBackBtn') this.showResetOptions();
      if (e.target.id === 'classListBackBtn') this.showResetOptions();
      if (e.target.id === 'studentListConfirmBtn') this.showStudentConfirmation();
      if (e.target.id === 'classListConfirmBtn') this.showClassConfirmation();
      if (e.target.id === 'resetAllConfirmYesBtn') {
        // Handled in showResetAllConfirmation
      }
      if (e.target.id === 'resetAllConfirmNoBtn') {
        // Handled in showResetAllConfirmation
      }
      if (e.target.id === 'studentConfirmYesBtn') {
        // Handled in showStudentConfirmation
      }
      if (e.target.id === 'studentConfirmNoBtn') {
        // Handled in showStudentConfirmation
      }
      if (e.target.id === 'classConfirmYesBtn') {
        // Handled in showClassConfirmation
      }
      if (e.target.id === 'classConfirmNoBtn') {
        // Handled in showClassConfirmation
      }
      if (e.target.id === 'selectAllStudentsBtn') {
        // Handled in showStudentList
      }
      if (e.target.id === 'selectAllClassesBtn') {
        // Handled in showClassList
      }
          ...
          ${isAnswered && !isLocked ? `
            <div style="margin-top: var(--spacing-lg); display: flex; gap: var(--spacing-md);">
              <button id="lockAnswerBtn" class="btn-primary" style="flex: 1;">
                🔒 Kunci Jawaban & Lihat Penjelasan
              </button>
            </div>
          ` : ''}
  ...
  showResetOptions() {
    ...
    document.getElementById('resetOptionsModal').addEventListener('click', (e) => {
  ...
  showStudentList() {
    ...
    // Close modal when clicking overlay
    document.getElementById('studentListModal').addEventListener('click', (e) => {
      if (e.target.id === 'studentListModal') {
        document.getElementById('studentListModal').remove();
      }
    });
  }
  ...
  showClassList() {
    ...
    // Close modal when clicking overlay
    document.getElementById('classListModal').addEventListener('click', (e) => {
      if (e.target.id === 'classListModal') {
        document.getElementById('classListModal').remove();
      }
    });
  }
